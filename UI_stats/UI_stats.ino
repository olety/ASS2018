...
int getTempFactor(int tmp){
  return (int)floor(abs(27-temp)/1.5);
}
...
//show the name
  M5.Lcd.setCursor(110,15);
  M5.Lcd.println(currentState == STATE_WIN ? "You Won" : "You Lost");
  M5.Lcd.setCursor(10,190);
...
//  M5.Lcd.print(currentState);
  ...
  M5.Lcd.print(getTempMessage(temp));
}
...
void updateScorePoints(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (currentState == STATE_IDLE) {
      sleep_pts = sleep_pts - 1 - getTempFactor(temp)*2;
      happiness_pts = happiness_pts - 1 - getTempFactor(temp)*2;
...
void lose() {
  vTaskDelete(idleHandle);
  vTaskDelete(scorePointsHandle);
  vTaskDelete(eventHandle);
